...
Two optional columns control animated zoom on image elements:

- _Zoom Effect_: `in` zooms from 100% to the zoom amount, `out` zooms from the zoom amount down to 100%. Leave empty for no zoom.
- _Zoom Amount_: peak zoom factor, e.g. `1.3` means 30% zoom. Valid range: 1–3. Defaults to 1.3 if omitted.
